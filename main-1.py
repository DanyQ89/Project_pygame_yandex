...
class Level():
    ...
    def draw(self):
        ...
        for layer in self.map:
            ...
            for tile in layer:
                if tile == '1':
                    display.blit(grass, (x * self.tile_size[0] - scrolling[0], y * self.tile_size[1] - scrolling[1]))
                if tile == '2':
                    display.blit(ground1, (x * self.tile_size[0] - scrolling[0], y * self.tile_size[1] - scrolling[1]))
                if tile == '3':
                    display.blit(ground2, (x * self.tile_size[0] - scrolling[0], y * self.tile_size[1] - scrolling[1]))
                if tile == '4':
                    display.blit(ground3, (x * self.tile_size[0] - scrolling[0], y * self.tile_size[1] - scrolling[1]))
                if tile == '5':
                    display.blit(ground4, (x * self.tile_size[0] - scrolling[0], y * self.tile_size[1] - scrolling[1]))
                if tile != '0':
                    rects_file.append(
                        pygame.Rect(int(x * self.tile_size[0]), int(y * self.tile_size[1]), self.tile_size[0],
                                    self.tile_size[1]))
                x += 1
            y += 1
...
class Player():
    ...
    def move(self):
        ...
        if self.jumping:
            ...
            for i in range(5):
                particles.append(Particle(player.rect.midbottom[0], player.rect.midbottom[1],
                                          [(150, 150, 150), (255, 255, 255), (200, 200, 200)], -40, 40, -5, 0, 2, 10,
                                          0.8, 0.2))
            self.jumping = False
        ...
        self.rect, self.collision_types = move(self.rect, rects_file, self.movement)

        self.vertical_momentum += strength_gravity
...
class Enemy():
    ...
    def move(self):
        ...
        self.rect, self.collision_types = move(self.rect,
                                               rects_file + [enemy.rect for enemy in enemies if enemy.id != self.id],
                                               self.movement)
    ...
    def attack(self):
        ...
        if math.sqrt(abs((self.rect.centerx - player.rect.centerx) ** 2 + (
            ...
            if self.shoot_timer >= 60:
                ...
                bullets_enemy.append(Projectile(self.rect.centerx + 5, self.rect.centery + 35, 15, 17, 50,
...
class Projectile():
    ...
    def update(self):
        self.trajectory()
        self.collision_check(self.rect, rects_file)
...
class Particle():
    def __init__(self, x, y, colors, min_xvel, max_xvel, min_yvel, max_yvel, min_radius, max_radius, shrink_rate,
                 gravity):
        self.x = x
        self.y = y
...
def move(rect, tiles, movement):
    ...
    rect.x += movement[0]
    hit_list = collision_check(rect, rects_file)
...
def draw():
    ...
    # display.blit(cloudsback, (0 - scrolling[0]*0.8 - 500, 0 - scrolling[1]*0.8 - 200))
    # display.blit(cloudsfront, (0 - scrolling[0]*0.6 - 500, 0 - scrolling[1]*0.6 - 200))
    # display.blit(bgback, (0 - scrolling[0]*0.4 - 500, 0 - scrolling[1]*0.4 - 200))
    # display.blit(bgfront, (0 - scrolling[0]*0.2 - 500, 0 - scrolling[1]*0.2 - 200))
...
while True:
    ...
    scrolling[0] += int((player.rect.x - scrolling[0] - (WINDOW_SIZE[0] / 2 + player.width / 2)) / 20)
    scrolling[1] += int((player.rect.y - scrolling[1] - (WINDOW_SIZE[1] / 2 + player.height / 2)) / 20)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and len(bullets) <= 20:
            ...
            bullets.append(
                Projectile(player.rect.centerx + 5, player.rect.centery + 35, 10, 14, 15, math.atan2(slopey, slopex),
                           projectile_img))
            shoot_sound.play()
    ...
    for bullet in bullets:
        if len(bullets) <= 30:
            ...
            if bullet.collision_types['top'] or bullet.collision_types['bottom'] or bullet.collision_types['right'] or \
                ...
                if bullet.collision_types['left']:
                    for i in range(20):
                        particles.append(Particle(bullet.x, bullet.y,
                                                  [(140, 140, 140), (200, 200, 200), (100, 100, 100), (123, 54, 0)], 10,
                                                  60, -25, 25, 4, 15, 0.4, 0.2))
        else:
            bullets.remove(bullet)

    # Пули врага
    for bullet in bullets_enemy:
        if len(bullets_enemy) <= 30:
            bullet.update()
            if bullet.rect.colliderect(player.rect):
                if bullet in bullets_enemy:
                    bullets_enemy.remove(bullet)
                player.health -= bullet.damage
                player_hit_sound.play()
                for i in range(20):
                    particles.append(
                        Particle(bullet.x, bullet.y, [(50, 50, 50), (180, 30, 30), (150, 150, 150), (94, 49, 91)], -25,
                                 25, -50, 0, 4, 15, 0.4, 0.2))
            if bullet.collision_types['top'] or bullet.collision_types['bottom'] or bullet.collision_types['right'] or \
                    bullet.collision_types['left'] and not bullet.rect.colliderect(player.rect):
                if bullet in bullets_enemy:
                    bullets_enemy.remove(bullet)
                explosion_sound.play()
                if bullet.collision_types['top']:
                    for i in range(23):
                        particles.append(
                            Particle(bullet.x, bullet.y, [(140, 140, 140), (100, 100, 100), (150, 54, 54)], -25, 25, 10,
                                     60, 4, 15, 0.4, 0.2))
                if bullet.collision_types['bottom']:
                    for i in range(23):
                        particles.append(
                ...
                if bullet.collision_types['right']:
                    for i in range(23):
                        particles.append(
                            Particle(bullet.x, bullet.y, [(140, 140, 140), (100, 100, 100), (150, 54, 54)], -60, -10,
                                     -25, 25, 4, 15, 0.4, 0.2))
                if bullet.collision_types['left']:
                    for i in range(23):
                        particles.append(
                            Particle(bullet.x, bullet.y, [(140, 140, 140), (100, 100, 100), (150, 54, 54)], 10, 60, -25,
                                     25, 4, 15, 0.4, 0.2))
        else:
            bullets_enemy.remove(bullet)
    ...
    if player.moving_right and player.collision_types['bottom']:
        particles.append(Particle(player.rect.midbottom[0], player.rect.midbottom[1],
                                  [(60, 163, 112), (61, 111, 112), (50, 62, 79), (94, 49, 91), (140, 63, 93),
                                   (186, 97, 86)], -50, 0, 0, 5, 2, 8, 0.4, 0.2))
    if player.moving_left and player.collision_types['bottom']:
        particles.append(Particle(player.rect.midbottom[0], player.rect.midbottom[1],
                                  [(60, 163, 112), (61, 111, 112), (50, 62, 79), (94, 49, 91), (140, 63, 93),
                                   (186, 97, 86)], 0, 50, 0, 5, 2, 8, 0.4, 0.2))
        ...
        if enemy.health <= 0 or enemy.rect.y >= 1080:
            ...
            for i in range(50):
                particles.append(Particle(enemy.rect.centerx, enemy.rect.centery,
                                          [(140, 140, 140), (255, 50, 50), (255, 50, 50), (255, 50, 50), (50, 50, 50),
                                           (100, 0, 0)], -40, 40, -80, 0, 4, 15, 0.4, 0.2))
        else:
            enemy.update()
